...
func (this *RiverHandler) GetNearestRivers(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		this.onError(w, err, fmt.Sprintf("Can not parse lat parameter: %s", lat_s), 400)
		return
	}
	lon_s := r.FormValue("lon")
	lon, err := strconv.ParseFloat(lon_s, 64)
	if err != nil {
		this.onError(w, err, fmt.Sprintf("Can not parse lon parameter: %s", lon_s), 400)
		return
	}
	point := Point{Lat:lat, Lon:lon}
	rivers, err := this.riverDao.NearestRivers(point, 5)
	if err != nil {
		this.onError500(w, err, "Can not select rivers")
		return
	}
