...
func (this *RouteHandler) EditRoute(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		this.onError500(w, err, "Can not edit route")
		return
	}
...
func (this *RouteHandler) DelRoute(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		this.onError500(w, err, "Can not remove tracks")
		return
	}
	err = this.storage.DeleteEventPointsForRoute(id)
	if err != nil {
		this.onError500(w, err, "Can not remove points")
		return
	}
	err = this.storage.DeleteRoute(id)
	if err != nil {
		this.onError500(w, err, "Can not remove route")
		return
	}
}
...
func (this *RouteHandler) writeRouteToResponse(id int64, w http.ResponseWriter) {
	...
	if !found {
		this.onError(w, fmt.Errorf("Route with id %d does not exist", id), "Not found", http.StatusNotFound)
	}
...
func (this *RouteHandler) RouteEditorPageHandler(w http.ResponseWriter, req *http.Request) {
	...
	if found {
		...
		if err != nil {
			this.onError500(w, err, "Can not serialize track")
			return
		}
		w.Write(bytes)
	} else {
		this.onError500(w, err, "Track not found")
	}
}
