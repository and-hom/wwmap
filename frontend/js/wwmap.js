...
function forceRedirect() {
    window.location.href = YNDX_AUTH_URL + "&redirect_uri=" + encodeURIComponent(window.location.href)
}
...
function getUserInfo(token) {
    ...
    xhr.open('GET', "http://localhost:7007/user-info?token=" + token, false);
    xhr.send();
    if (xhr.status == 200) {
...
function authControlHtml(authorizedTemplate, anonymousTemplate) {
    token = getToken()
    if (!token) {
        token = extractToken()
    }
    if (token) {
        ...
        try {
            ...
            return tim(authorizedTemplate, userInfo);
         } catch (err) {
            console.error(err)
