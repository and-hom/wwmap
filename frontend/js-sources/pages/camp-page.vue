<template>
    <page link="camp.htm">
        <div v-if="canEdit">
            <h2 style="display: inline;">Стоянки</h2>
            <button data-toggle="modal" data-target="#add-camp" style="margin-left:30px;">+</button>
        </div>
        <table class="table">
            <thead>
            <tr>
            ...
            <tbody>
            <tr :id="`row${camp.id}`" :ref="`row${camp.id}`" v-for="camp in camps"
                :class="rowClass(camp)">
                <td>{{camp.title}}</td>
                <td>
                    <ul class="ti-tags">
                        <li v-for="station in camp.stations" class="ti-tag">
                ...
                <td class="raw-text-content">{{camp.description}}</td>
                <td v-if="canEdit">
                    <button v-on:click="transferForEdit={ ...camp }"
                            data-toggle="modal" data-target="#add-camp">Правка
                    </button>
                    <ask :id="'del-camp-'+camp.id" title="Удалить задачу?"
                    ...
                    <button data-toggle="modal" :data-target="'#del-camp-'+camp.id">Удалить</button>
                </td>
            </tr>
            </tbody>
        </table>
...
<script>
    ...
    export default {
        ...
        methods: {
            refresh: function () {
                doGetJson(backendApiBase + "/camp", false).then(camps => {
                    this.camps = camps;
                    this.stations = Array.from(new Set(camps.flatMap(t => t.stations)));
    ...
    resetTransferForEdit: function () {
            ...
            remove: function (id) {
                doDelete(backendApiBase + "/camp/" + id, true).then(this.refresh)
            },
            ...
            rowClass: function (camp) {
                return `${camp.id}` == this.selected ? 'row-selected' : '';
            }
        }
    }
</script>
